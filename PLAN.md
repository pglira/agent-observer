...
| Bottom line | A `bottom_line_width`px line (default 5, `colors.line` blue) along the bottom of the bar (CSS `border-bottom` on `#rowbox`). |
...
| Label content | Configurable `label_format` Pango-markup template. Placeholders: `{idx} {project} {title} {status} {uptime} {pid} {cwd} {dc}` (values auto-escaped). `{idx}` renders as `[1]`..`[0]`. The base text color is applied via markup (not CSS) so the focused-name color can override per-run. |
